...
def show_zoom_navigable(img_path):
    ...
    canvas.bind("<MouseWheel>", zoom_mouse)  # Windows/Mac
...
if not (os.path.exists(types_path) and os.path.exists(types_path2)):
    ...
    try:
        img = Image.open(types_path).resize((450, 300))
        img = ImageTk.PhotoImage(img)

        img_label = Label(scroll_frame, image=img, bg="white")
        img_label.image = img
        img_label.place(x=100, y=y_position, width=450, height=300)
        y_position += 300 + 10

        img2 = Image.open(types_path2).resize((450, 200))
        img2 = ImageTk.PhotoImage(img2)

        img_label2 = Label(scroll_frame, image=img2, bg="white")
